...
fn convolution_1d_v1(data_in:& Vec<f64>) -> Vec<f64>{
    ...
    while j < max_index{
        ...
        let mut binom_normalization_const = 1.0;
        let mut sum_sum = 0.0;
        

        // forward iteraion part. from `j` to `N`
        let factor = prob / (1.0-prob);
        let mut prev   = 1.0;
        let mut k=j;
        while k < max_index{
            let binom     = prev * _forward_factor[k] * factor;
            binom_normalization_const += binom;
...
//            cout << binom << ", ";
        ...
        data_out[j] = sum_sum / binom_normalization_const;
        if j % step == 0 {
...
fn convolution_j1_to_j2(j1:usize, j2:usize, step:usize,
    ...
    // println!("Enter convolution_j1_to_j2 for ({j1},{j2}) pair");
    let mut j = j1;
    let size_n = data_in.len();
    while j < j2{
    ...
    // println!("Exit convolution_j1_to_j2 for ({j1},{j2}) pair");
}
...
fn convolution_1d_v2(data_in:& Vec<f64>, threads:usize) -> Vec<f64>{
    ...
    // println!("Exiting convolution_1d_v2");
    data_out
}
